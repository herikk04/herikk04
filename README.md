# Olá, sou Herik Kauan! 👋

**Estudante de Inteligência Artificial** | **Desenvolvedor** | **Entusiasta de Tecnologia e Inovação**

<p align="left">
  <a href="https://www.linkedin.com/in/herik-kauan-assis/">
    <img src="https://img.icons8.com/fluency/48/000000/linkedin.png" alt="LinkedIn" width="40"/>
  </a>
  <a href="mailto:herikkauan04@gmail.com">
    <img src="https://img.icons8.com/fluency/48/000000/gmail.png" alt="Email" width="40"/>
  </a>
   <a href="https://huggingface.co/HerikKkk">
    <img src="https://upload.wikimedia.org/wikipedia/commons/4/46/Hugging_Face_logo.png" alt="Hugging Face" width="40"/>
  </a>
</p>

---

## Sobre Mim

Sou estudante de **Inteligência Artificial** na **UFG** e **Técnico em Informática para Internet** pelo **IFG**. 

---

## 🚀 Habilidades

- **Linguagens:** Python, Java, PHP, JavaScript, HTML/CSS
- **Ferramentas:** Git, Docker, DBeaver
- **Bancos de Dados:** PostgreSQL, MySQL

---

## 🌱 Aprendendo

- **Modelos Avançados de Machine Learning**
- **Integração de Bancos de Dados Vetoriais e de Grafos**
- **Desenvolvimento de Assistentes Virtuais com IA**

---

## ⚡ Curiosidades

- Apaixonado por **empreendedorismo**, **tecnologia** e **fotografia**.
- Busco sempre equilíbrio entre **vida profissional** e **bem-estar pessoal**.
- Gosto de explorar **filosofia** e refletir sobre diferentes visões da vida.
